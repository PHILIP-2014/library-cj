	...
	function page(pageNum,totalRecord){
	    totalPageCount = totalRecord % 12 == 0 ? parseInt(totalRecord/ 12) : parseInt(totalRecord / 12) + 1;
		pageNum=pageNum;
	...
	function find(obj){
		...
		$("tbody tr").hide();
		$("tbody tr:gt("+((a-1)*12)+")").show();
		$("tbody tr:gt("+((a-1)*12+12)+")").hide();
	...
	$(document).ready(function(){
		...
		var numc= nums % 12 == 0 ? parseInt(nums/ 12) : parseInt(nums / 12) + 1;
		$(".page85").text(numc);
		$(".page500").text(nums);
